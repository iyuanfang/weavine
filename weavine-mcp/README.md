...
|----------|------------------------------|-------|
| Default  | (unset) or `WEAVINE_MCP_TIER=default` | 37 (contact, event, action, project, reminder, note, quick) |
| Full     | `WEAVINE_MCP_TIER=full`      | 57 (adds auth_jwt, diagnostic, tag, interaction, setting, search, sync) |
...
note (Tier 1)           list / get / create / update / delete /
                        list_note_backlinks / list_note_entity_links
quick (Tier 1)          quick_parse
...
sync (Tier 2)           sync_manifest / sync_pull
```
...
- `src/client.rs`           — reqwest + X-API-Key + retry + typed
                              `get_json` / `post_json` / `put_json`
                              wrappers; post_public / delete_with_body
                              for endpoints needing them
- `src/server.rs`           — manual call_tool dispatch + list_tools;
                              Tier 1 always-on, Tier 2 opt-in; `parse()`
                              rejects empty `*_id` / `id` / `*_ids` fields
...
- `src/tools/note.rs`       — Tier 1 (7) — CRUD + backlinks + entity_links
- `src/tools/quick.rs`      — Tier 1 (1) — Ctrl+K-style free-form parse
...
- `src/tools/sync.rs`       — Tier 2 (2)

All tool handlers carry `#[tracing::instrument(skip_all, fields(tool
= stringify!(name)))]`, so every MCP call shows up as a span named
after the handler.
