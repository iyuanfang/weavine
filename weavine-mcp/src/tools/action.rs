...
impl WeavineMcpServer {
        ...
        let v = self.client.get("/api/actions", &refs).await?;
        Ok(v)
    }
        ...
        let v = self.client.get(&format!("/api/actions/{}", input.id), &[]).await?;
        Ok(v)
    }
        ...
        let v = self.client.post("/api/actions", &body).await?;
        Ok(v)
    }
        ...
        let v = self.client.put(&format!("/api/actions/{id}"), &body).await?;
        Ok(v)
    }
        ...
        self.client.delete(&format!("/api/actions/{}", input.id)).await?;
