...
pub async fn query(
    ...
    let events = sqlx::query_as::<_, Event>(
        "SELECT e.id, e.user_id, e.title, e.event_type, e.start_at, e.end_at, e.location, e.notes, \
         ...
         WHERE e.user_id = $1 AND (e.title ILIKE $2 OR e.notes ILIKE $2)",
    )
    .bind(&auth).bind(&pattern)
    .fetch_all(&*pool).await.unwrap_or_default();

    let actions = sqlx::query_as::<_, Action>(
        "SELECT a.id, a.user_id, a.title, a.description, a.status, a.priority::BIGINT AS priority, \
         ...
         WHERE a.user_id = $1 AND (a.title ILIKE $2 OR a.description ILIKE $2)",
    )
    .bind(&auth).bind(&pattern)
    .fetch_all(&*pool).await.unwrap_or_default();
    ...
    let projects = sqlx::query_as::<_, Project>(
        "SELECT id, user_id, title, description, template, stage, \
         ...
         FROM project WHERE user_id = $1 AND (title ILIKE $2 OR description ILIKE $2)",
    )
    .bind(&auth).bind(&pattern)
    .fetch_all(&*pool).await.unwrap_or_default();
