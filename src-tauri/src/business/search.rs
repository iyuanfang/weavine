...
pub fn search(
    ...
    // Every entity query MUST include `deleted_at IS NULL` so soft-deleted rows
    // never leak into the global search results (matches server-side filter in
    // server/src/handlers/search.rs and the list()/get() filters in this crate).
    let deleted_clause = " AND deleted_at IS NULL";
    ...
    let interactions: Vec<Interaction> = {
        let mut stmt = conn.prepare(
            "SELECT Interaction.id, Interaction.user_id, Interaction.contact_id, Interaction.action_id, Interaction.event_id, Interaction.occurred_at, Interaction.channel, Interaction.summary, Interaction.source, Interaction.source_ref, Interaction.created_at, Interaction.deleted_at, c.nickname AS contact_nickname \
    ...
    let events: Vec<Event> = {
        let sql = format!(
            "SELECT Event.id, Event.user_id, Event.title, Event.event_type, Event.start_at, Event.end_at, Event.location, Event.contact_id, Event.project_id, Event.reminder_lead_minutes, Event.archived_at, Event.created_at, Event.updated_at, Event.deleted_at, c.nickname AS contact_nickname, p.title AS project_title \
    ...
    let actions: Vec<Action> = {
        let sql = format!(
            "SELECT Action.id, Action.user_id, Action.title, Action.status, Action.priority, Action.category, Action.due_at, Action.contact_id, Action.project_id, Action.completed_at, Action.archived_at, Action.created_at, Action.updated_at, Action.deleted_at, c.nickname AS contact_nickname, p.title AS project_title \
    ...
    let projects: Vec<Project> = {
        let sql = format!(
            "SELECT id, user_id, title, template, stage, start_at, due_at, completed_at, archived_at, created_at, updated_at, deleted_at \
             FROM \"Project\" WHERE user_id = ?1 \
             AND (title LIKE ?2){}{} \
             ORDER BY updated_at DESC LIMIT ?3",
            archive_clause, deleted_clause
        );
        let mut stmt = conn.prepare(&sql)?;
        let results = stmt
    ...
    let notes: Vec<Note> = {
        let sql = format!(
            "SELECT id, user_id, title, substr(body, 1, 200), archived_at, created_at, updated_at, imported_from, imported_at, deleted_at \
