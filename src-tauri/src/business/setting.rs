...
pub fn list(conn: &Connection, owner_id: &str) -> rusqlite::Result<Vec<Setting>> {
...
pub fn upsert(conn: &Connection, owner_id: &str, key: &str, value: &str) -> rusqlite::Result<Setting> {
...
pub fn delete(conn: &Connection, owner_id: &str, key: &str) -> rusqlite::Result<()> {
    conn.execute(
